...
public class ParonymFileLoader {
    ...
    public void loadCodes(String codesFileName) throws Exception {
        BufferedReader codesFile = new BufferedReader(new FileReader(codesFileName));
        String s;
        StringTokenizer tok;
        String tableName;
        int code;
        ...
        try {
            while ((s = codesFile.readLine()) != null) {
                ...
                str = tok.nextToken();
                db.update("insert into " + tableName + "(code,str) values (" + code + ",'" + str + "')");
            }
    ...
    * работает только с существительными. часть речи должна быть заранее определена
    * removes paronyms and words_morphs containing removed words
    ...
    */
    public void removePluralNouns() throws Exception {
        ...
        BufferedWriter logfile = new BufferedWriter(new FileWriter("removePlural.txt"));

        System.out.println("Removing plural nouns");
        try {
            //for all words
            rs = db.queryResult("select * from words"); //@todo maybe too large ResultSet
            while (rs.next()) {
    ...
    //осуществляет подсчет буквенных и морфемных расстояний методом сопоставления «каждого с каждым»
    public void addAllParonyms(int dlLimit, int dmLimit, int drLimit) throws Exception {
        ResultSet rs;
        boolean found = false;
        int counter = 0;
        ...
        while (rs.next()) {
            ...
            wordList.add(new WordFromFile(id, rs.getString("word"), rs.getInt("part"), num, Query.constructMorphList(db, id)));
        }
        rs.close();
        for (int i = 0; i < wordList.size(); i++) {
            ...
            for (int j = i + 1; j < wordList.size(); j++) {
                ...
                if ((word1.num != word2.num) && (word1.num != -1) && (word2.num != -1)) {
                    continue;
                }
                ...
                if ((dlcode != -1)||(dmcode != -1)) {
                    dl = dlcode == -1 ?
                            Distance.countDistance(word1.word, word2.word, false) : ParonymCoder.getDl(dlcode);
                    q = Distance.countFormula(word1.word, word2.word, dl);

                    found = db.queryResult("select * from paronyms where " +
                            "(id1=" + id1 + ") and(id2=" + id2 + ") union all " +
    ...
    int getDmCode(WordFromFile word1, WordFromFile word2, int dmLimit, int drLimit) throws Exception {
        if (dmLimit < 1) {
            return -1;
        }
        ...
        return ParonymCoder.generateDmCode(dm, dr, pref, root, relRoots, suff, end);
    }

    byte getDlCode(WordFromFile word1, WordFromFile word2, int dlLimit) throws ParonymCodeException {
        if (dlLimit < 1) {
            return -1;
        }
    ...
    private boolean relativeRoots(Morph root1, int part1, Morph root2, int part2) throws SQLException {
        int rootId1 = root1.id_m;
        int rootId2 = root2.id_m;
        return db.queryResult("select * from roots where " +
                "(id1=" + rootId1 + ") and(id2=" + rootId2 + ")and(part=" + part1 + ") union " +
                "select * from roots where (id1=" + rootId2 + ") " +
                "and(id2=" + rootId1 + ")and(part=" + part2 + ")").next();
    }

    private HashSet<Integer> getDifferMorphTypes(ArrayList<ArrayList<Morph>> diff) throws Exception {
        Morph m;
    ...
    public void fillGramFields() throws Exception {
        ResultSet rs;
        BufferedWriter file = new BufferedWriter(new FileWriter("addGram_WordsNotFound.txt"));
        boolean found;
        int counter = 0;
    ...
    private boolean trySetGramFields(WordFromFile w, ArrayList<Paradigm> paradigms) throws SQLException {
        boolean found = false;
        ...
        for (Paradigm p : paradigms) {
            if (w.part == p.part) {
                ...
                if (MorphAn.decParam(w, p.params + (p.paramForms.isEmpty() ? "" : p.paramForms.get(0)))) { //false, если был конфликт параметров
