...
public class Query {
    ...
    // получить из базы данных строковое предст. корня. корень задается своим
    // id_m
    ...
    public static ArrayList<Morph> constructMorphList(Database db, int id)
            throws SQLException {
        ...
        rs = db.queryResult("select * from\n"
                + "words_morphs wm join morphs m on wm.id_m = m.id_m where wm.id ="
                + id);
        // @todo проверять порядок следования морфов
        while (rs.next()) {
            morphId = rs.getInt("id_m");
            morphList.add(new Morph(morphId, rs.getInt("type"), rs
                    .getString("morph")));
        }
    ...
    public static int getMorphType(Database db, int morphId)
            throws SQLException {
        ResultSet rs;
