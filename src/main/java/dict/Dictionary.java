...
public class Dictionary implements DictionaryInterface {
    Database db;
    final static QueryParameters DEFAULT_QUERY_PARAMETERS = new QueryParameters(
            -1, -1, -1, 0.3, false, true, false, false, false, true, true,
    ...
    public ArrayList<ParonymList> executeQuery(String word,
            QueryParameters queryParams) throws Exception {
    ...
    public ArrayList<ParonymList> findWordNormParonyms(String word,
            QueryParameters queryParams) throws Exception {
        ArrayList<ParonymList> paronymLists = new ArrayList<ParonymList>();
        ResultSet rs;
        boolean found = false; // true, если заданное слово найдено в бд
        Word w;
        String notAot = queryParams.includeNotFound ? ""
                : "and(aot_id is not null)";
        String setPart = queryParams.setPart == -1 ? "" : "and(part = "
                + queryParams.setPart + ")";

        rs = db.queryResult("select * from words where (word ='" + word + "') "
                + notAot + setPart);
        while (rs.next()) {
            found = true;
        ...
        if (!found) {
            throw new ParonymsNotFoundException("Слово «" + word
                    + "» не найдено");
        }
        return paronymLists;
    }
    ...
    /**
     * поиск паронимов по бд для заданной словоформы word с помощью аот для
     * словоформы находятся все варианты нормальных форм, для каждой нормальной
     * формы - все варианты наборов граммем (например: пекло. Норм. формы:
     * печь(гл) ; пекло(сущ): им.п., вин.п.) для каждого варианта ищутся
     * паронимы по бд с помощью аот паронимы возвращаются в ту же форму, что и
     * исходное слово
     * 
     * @param word
     *            исходная словоформа
    ...
    public ArrayList<ParonymList> findWordFormParonyms(String word,
            QueryParameters queryParams) throws Exception {
        ArrayList<ParonymList> paronymLists = new ArrayList<ParonymList>();
        ResultSet rs;
        Word w;
        ...
        if (paradigms.isEmpty()) {
            throw new ParonymsNotFoundException("Словоформа «" + word
                    + "» не найдена");
        }
        for (Paradigm p : paradigms) {
            ...
            rs = db.queryResult("select * from words where (aot_id ="
                    + p.aot_id + ")and(word='" + p.norm + "')" + notAot);
            if (!rs.next()) {
                continue;
            }
            ...
            for (String gram : p.paramForms) {
                ...
                paronymLists.add(new ParonymList(w,
                        findParonyms(w, queryParams)));
            }
        }
    ...
    public ArrayList<Paronym> findParonyms(Word w, QueryParameters queryParams)
            throws Exception {
        ...
        // поиск паронимов
        rs = db.queryResult("select par.*, d.* " + "from paronyms d  "
                + "join words par on (par.id=d.id2) " + "where (d.id1= " + w.id
                + ")" + partQuery + genQuery + numQuery + notAot +

                "union " +

                "select par.*, d.* " + "from paronyms d  "
                + "join words par on (par.id=d.id1) " + "where (d.id2=" + w.id
                + ")" + partQuery + genQuery + numQuery + notAot);
     ...
     * @param rs
     * @param gram
     *            grammatical form parameters of the initial word
     * @param sameGen
     * @param sameNum
     * @return
     * @throws SQLException
     */
    public Word makeWord(ResultSet rs, String gram, boolean sameGen,
            boolean sameNum) throws Exception {
        String word;
        int id;
        int gen; // gen
        int num; // num
        ...
        if (gram.equals("")) {
            ...
            if (aot_id == -1) {
                ...
                form = MorphAn.constructForm(aot_id, gram, gramCut(gram),
                        sameGen);
                if (form.equals("")) {
                    return null;
                }
        ...
        return new Word(word, id, aot_id, splitWord, rs.getInt("part"), gen,
                num, anim, form, gram, rootId, morphList);
    ...
    private String gramCut(String gram) {
        ...
        while (tok.hasMoreTokens()) {
            gr = tok.nextToken();
            if (!gr.equals("мр") && !gr.equals("жр") && !gr.equals("ср")
                    && !gr.equals("мр-жр")) {
    ...
    public Paronym makeParonym(Word qword, ResultSet rs,
            QueryParameters queryParams) throws Exception {
        Paronym p;
        ...
        byte dlcode = rs.getObject("dlcode") == null ? -1 : rs
                .getByte("dlcode");
        ...
        if (queryParams.enableForms) {
            params.curDl = Distance.countDistance(qword.form, w.form,
                    queryParams.enableDl2);
    ...
    /**
     * Осуществляет проверку соответствия параметров паронимов параметрам
     * запроса. По результатам проверки в p заполняются поля l_ok и m_ok
     * 
     * @param p
     *            Параметры паронимической пары
     * @param qp
     *            Параметры запроса
     */
    ...
    private boolean checkRoot(Word paronym, Word qword,
            QueryParameters queryParams) throws SQLException {
        ...
        if (!queryParams.diffRoot && queryParams.relativeRoots) {
            ...
            if ((rootId1 != rootId2)
                    && (!db.queryResult(
                            "select * from roots where " + "(id1=" + rootId1
                                    + ") and(id2=" + rootId2 + ")and(part="
                                    + paronym.part + ") union "
                                    + "select * from roots where (id1="
                                    + rootId2 + ") " + "and(id2=" + rootId1
                                    + ")and(part=" + paronym.part + ")").next())) {
    ...
    public ArrayList<CompareForms> executeCompareQuery(String form1,
        ...
        if (paradigms1.isEmpty() || paradigms2.isEmpty()) {
            ...
            for (Word w1 : list1) {
                for (Word w2 : list2) {
                    rs = db.queryResult("select * " + "from paronyms d "
                            + "where (d.id1= " + w1.id + ")and(d.id2 = "
                            + w2.id + ")" + "union " + "select * "
                            + "from paronyms d  " + "where (d.id1=" + w2.id
                            + ")and(d.id2 = " + w1.id + ")");
                    if (rs.next()) {
    ...
    private ArrayList<Word> composeWordList(ArrayList<Paradigm> paradigms)
            throws Exception {
    ...
    public static void massQuery(Dictionary dict) throws Exception {
        ...
        System.out.println((double) 1000 * max
                / (double) (time2.getTime() - time1.getTime())
                + " sigle queries per second");
