...
/**
 * Поддержка работы с морфологическим анализатором.
 * Функции морфологического анализа осуществляются обращением к методам класса NativeAot.
 * Выполняется разбор строк, возвращаемых морф. анализатором, и
 * запись полученных данных в структуру (список структур) Paradigm
 */
public class MorphAn {
    ...
    /**
     * Морфологический анализ входной словоформы, интерпретируемой как существительное.
     * @param s Входная словоформа
     * @param asNorm Интерпретировать слово как нормальную форму (true)/ произвольную словоформу (false)
     * @return Возвращается первая подходящая парадигма (объект типа Paradigm). В случае неуспеха возвращается null.
     * @throws MorphAnException Ошибка морфологического анализатора
     */
    private static Paradigm analyzeAsNoun(String s, boolean asNorm) throws MorphAnException {
        //найти все морфологические интерпретации словоформы
        ArrayList<Paradigm> paradigmsList = asNorm ? analyzeNorm(s) : analyzeForm(s);
    ...
    /**
     * Морфологический анализ входной канонической словоформы, интерпретируемой как существительное.
     * @param s Входная словоформа
     * @return Первая подходящая парадигма. В случае отсутствия подходящих парадигм - null.
     * @throws MorphAnException Ошибка морфологического анализатора
     */
    ...
    /**
     * Морфологический анализ входной произвольной словоформы. Словоформа интерпретируется как существительное.
     * @param s Входная словоформа
     * @return Первая подходящая парадигма. В случае отсутствия подходящих парадигм - null.
     * @throws MorphAnException Ошибка морфологического анализатора
     */
     ...
     * @throws MorphAnException Ошибка морфологического анализатора
     */
    private static ArrayList<Paradigm> analyzeWord(String s, boolean asNorm) throws MorphAnException {
        Paradigm w;
        String gramStr = "";
        ...
        try {
            ...
            gramStr = asNorm ? aot.getParadigmsFromNorm(s) : aot.getParadigmsFromForm(s);
            ...
            while (tok.hasMoreTokens()) {
                ...
                //нормальная форма. в режиме нормальных форм в строке не содержится
            ...
            throw new MorphAnException("Invalid Aot string format. \n" +
    ...
    /**
     * Морфологический анализ словоформы, интерпретируемой как каноническая форма.
     * @param s Входная словоформа
     * @return Список всех найденных парадигм
     * @throws MorphAnException Ошибка морфологического анализатора
     */
    public static ArrayList<Paradigm> analyzeNorm(String s) throws MorphAnException {
        return analyzeWord(s, true);
    }

    /**
     * Морфологический анализ словоформы, интерпретируемой как произвольная словоформа.
     * @param s Входная словоформа
     * @return Список всех найденных парадигм
     * @throws MorphAnException Ошибка морфологического анализатора
     */
    public static ArrayList<Paradigm> analyzeForm(String s) throws MorphAnException {
     ...
     */
    public static String constructForm(int aot_id, String gram, String gramCut, boolean sameGen) {
        return aot.getFormFromParadigmIdAndGram(aot_id, gram, gramCut, sameGen);
    }

    /**
     * Преобразование граммемы части речи, используемой в АОТ, в числовой код, используемый в словаре паронимов.
     * Помимо преобразования кода выполняется обобщение частей речи.
     * Коды частей речи должны соответствовать входному файлу codes.txt
     * @param posAot символьный код части речи, используемый в АОТ
     * @return числовой код части речи, используемый в словаре паронимов, -1 в случае неуспеха
     */
    public static int posAotToInt(String posAot) {
        ...
        if (posAot.equals("С") || posAot.equals("МС")) {
            ...
            return NUMER;
                ...
                posAot.equals("МЕЖД") || posAot.equals("ЧАСТ") || posAot.equals("ВВОДН")) {
            return FUNC;
        } else {
            if (!posAot.equals("ФРАЗ")) {
                System.out.println("posAotToInt: Unknown part of speech: " + posAot);
            }
            return -1;
        }
    }
     ...
     * Производится сопоставление параметров из строки, выданной АОТом
     * с параметрами, уже приписанными слову.
     * @param w слово
     * @param paramStr строка морф. параметров, выданная АОТом
     * @return true , если в параметрах нет конфликтов, и false, если есть конфликты.
     */
    public static boolean decParam(WordFromFile w, String paramStr) {
        ...
        //Переменная animconf отражает наличие конфликтов в параметрах одушевленности и
        ...
        //Сопоставление параметров производится последовательным просмотром строки paramStr
        ...
        while (tok.hasMoreTokens()) {
            ...
            //при сопоставлении параметров сначала запрашивается значение соответствующего флага, оно может равняться:
            //FINAL - морф. параметр слова был определен до начала текущего вызова метода decParam
            //SET  - морф. параметр был приписан слову в результате анализа предшествующих элементов строки paramStr
            //NOTSET - морф. параметр не определен
            //в зависимости от значения флага и текущих морф. параметров, приписанных к слову,
            // сохраняется либо добавляется морф. параметр или сигнализируется конфликт
            if (param.equals("мр")) {
                switch (w.genflag) {
                    case FINAL:
                        if ((w.gen != M) && (w.gen != MF)) {
                    ...
                    case SET:
                        if (w.gen == S) {
                            genConflict = true;
                            System.out.println("Conflict: мр " + w.gen + " " + w.word + " " + param);
                        } else if (w.gen == F) {
                            w.gen = MF;
                        }
                }
                ...
                switch (w.genflag) {
                    ...
                    case SET:
                        if (w.gen == S) {
                            genConflict = true;
                            System.out.println("Conflict: жр " + w.gen + " " + w.word + " " + param);
                        } else if (w.gen == M) {
                            w.gen = MF;
                        }
                }
                ...
                switch (w.genflag) {
                    ...
                    case SET:
                        if (w.gen != S) {
                            genConflict = true;
                            System.out.println("Conflict: cр " + w.gen + " " + w.word + " " + param);
                        }
                }
                ...
                switch (w.genflag) {
                    ...
                    case SET:
                        if (w.gen == S) {
                            genConflict = true;
                            System.out.println("Conflict: мр-жр " + w.gen + " " + w.word + " " + param);
                        }
                }
